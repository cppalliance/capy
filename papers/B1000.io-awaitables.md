...
To understand our design choices, we must first understand the `std::execution` model ([P2300](https://wg21.link/p2300)) and its architectural assumptions.
...
The critical architectural issue: the sender queries the receiver's environment *after* connection to discover context—scheduler, allocator, stop token. [P2300R10](https://wg21.link/p2300) states explicitly:
...
```mermaid
flowchart TB
    subgraph SenderReceiver["std::execution (Backward Query)"]
        direction LR
    ...
    subgraph IoAwait["IoAwaitable (Forward Flow)"]
        direction LR
...
Whereas _IoAwaitable_ allows _receiving_ propagated context, the related refinement _IoAwaitableTask_ adds the promise interface needed to **store** received context and **propagate** it to child coroutines (the same storage methods also enable direct injection from launch functions):
...
Coroutines can also access their context directly using `this_coro::executor` and `this_coro::stop_token`. These never suspend—they return immediately with the stored values:
...
### 4.1 Context Reception

When a parent coroutine awaits a child, context flows through a three-argument `await_suspend`:
...
struct my_task
{
    ...
    // This signature enables context reception
...
While _IoAwaitable_ handles context *reception*, the _IoAwaitableTask_ refinement adds the promise interface needed to *store and propagate* that context. This involves six promise methods:
...
A task exposes these methods for launching:
...
### 4.5 Different Design Tradeoffs
...
> **Non-normative note.** The `io_awaitable_support` CRTP mixin (§9.1) provides all six required promise methods and is offered as a convenience. It is not proposed for standardization—implementors may write the boilerplate themselves.
