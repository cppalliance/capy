...
Ask a room of C++ developers: "How would you design a function that sends bytes over a network?" The answers arrive with remarkable consistency:
...
Simple. Modern. Type-safe. What's not to like?

Then the edge cases emerge:

```cpp
// "But what about string literals?"
void send(std::string_view data);  // overload 2

// "And raw arrays from C APIs?"
void send(const void* data, size_t len);  // overload 3

// "And multiple buffers for scatter/gather?"
void send(std::span<const std::byte>* buffers, size_t count);  // overload 4
```

The proliferation of overloads is the tell. Each new signature represents a failure to see the underlying abstraction. The designer is playing whack-a-mole with use cases instead of identifying the concept that unifies them.

This is the span reflex: reaching for a concrete type because it's familiar and "simple," without asking what abstraction would enable composition.

---

## 2. The Concept-Driven Alternative

Boost.Asio, after twenty years of production use, arrived at a different answer. Its `send` operations accept any type modeling the `ConstBufferSequence` concept:
...
- A single `std::span<const std::byte>`
- A `std::string_view`
- A `std::array<std::byte, N>`
- A `std::vector<mutable_buffer>` (scatter/gather)
- A custom buffer chain type
- Any user-defined type that provides `begin()` and `end()` returning buffer iterators

The concept describes *what operations are valid*, not *what type you must use*. This enables composition without the library author anticipating every use case.
...
The second question leads to composable designs. The first leads to overload proliferation.

---

## 3. The Allocation Cost of Span Composition

The span reflex extracts a concrete cost. Suppose you need to send an HTTP response: a header followed by a body—two non-contiguous memory regions. With span-based design:

```cpp
// Option A: Allocate storage for the span array
std::vector<std::span<const std::byte>> buffers;
buffers.push_back(as_bytes(header));
buffers.push_back(as_bytes(body));
send(buffers.data(), buffers.size());  // runtime allocation

// Option B: Copy into contiguous buffer
...
send(combined);  // allocation + copy
```

With concept-based design:

```cpp
// No allocation - composition happens in the type system
struct HeaderAndBody {
    std::span<const std::byte> header;
    std::span<const std::byte> body;
    
    auto begin() const { return buffer_iterator{this, 0}; }
    auto end() const { return buffer_iterator{this, 2}; }
};

send(HeaderAndBody{as_bytes(header), as_bytes(body)});  // zero allocation
```

The key insight: **composing spans requires runtime allocation because span is a concrete type. Composing types that model buffer concepts requires no allocation because the composition happens at compile time.**

This isn't abstract philosophy. It's the difference between zero-copy scatter/gather I/O and unnecessary allocations on every send. In high-performance networking code, this difference matters enormously.

---

## 4. Why Span Feels "Right" But Isn't
...
## 5. The void* Problem: When byte Is an Opinion
...
- Use a concept (like Asio's buffer types) that abstracts over the element type
...
## 7. Historical Parallel: The STL's Founding Insight
...
The span reflex is a regression from this insight. It's as if, after thirty years of generic programming, we forgot why it matters. The developer who reaches for `span<byte>` instead of a buffer concept has unlearned the lesson Stepanov taught.
...
## 9. Case Study: Spans at the Virtual Boundary
...
**The discovery**: When exploring how to structure a modern networking library built around C++20 coroutines, I found that spans *are* the natural representation of buffer sequences—but only at a specific architectural layer: the virtual boundary.
...
At this boundary, concept-based flexibility provides no benefit—you must pick a concrete representation. And spans (or an array of spans for scatter/gather) are precisely that representation:

```cpp
// The virtual interface - spans are correct here
class socket_stream {
public:
...
task<size_t> read_some(socket_stream& stream, Buffers&& buffers) {
    // Convert concept-modeled type to spans at the virtual boundary
    for (auto buf : buffer_sequence_view(buffers)) {
...
1. **User-facing APIs** accept any type modeling the buffer concept (composition, zero allocation)
2. **Internal virtual boundaries** use spans (necessary type erasure)
...
## 10. The Opposite Failure: Over-Engineered Abstraction
