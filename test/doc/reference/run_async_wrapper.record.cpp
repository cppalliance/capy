...
void correct_usage( any_executor ex )
{
    // Correct usage - wrapper is temporary, task is the direct argument
    run_async(ex)(my_task());
}

void rvalue_only_call( any_executor ex )
{
    ...
    static_assert(   std::is_invocable_v< wrapper,  task<void> > );
    static_assert( ! std::is_invocable_v< wrapper&, task<void> > );

    std::move(w)(my_task());  // Compiles: w is now an rvalue
}

void silent_misuse( any_executor ex )
{
