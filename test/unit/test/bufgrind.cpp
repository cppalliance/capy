    ...
    testMutableBuffer()
    {
        ...
        auto r = f.inert([&](fuse&) -> task<> {
            ...
            while(bg) {
                ...
                // A slice of a mutable input models MutableBufferSequence
                static_assert(MutableBufferSequence<decltype(b1)>);
                static_assert(MutableBufferSequence<decltype(b2)>);
                BOOST_TEST_EQ(buffer_size(b1) + buffer_size(b2), 5u);
            }
        });
        BOOST_TEST(r.success);
    }
    ...
    testConstBuffer()
    {
        ...
        auto r = f.inert([&](fuse&) -> task<> {
            ...
            while(bg) {
                ...
                // A slice of a const-only input models ConstBufferSequence
                // but not MutableBufferSequence.
                static_assert(ConstBufferSequence<decltype(b1)>);
                static_assert(!MutableBufferSequence<decltype(b1)>);
                static_assert(ConstBufferSequence<decltype(b2)>);
                static_assert(!MutableBufferSequence<decltype(b2)>);
                BOOST_TEST_EQ(buffer_size(b1) + buffer_size(b2), 5u);
            }
        });
    ...
    testWithReadStream()
    {
        ...
        auto r = f.inert([&](fuse&) -> task<> {
            ...
            while(bg) {
                ...
                if(buffer_size(b1) > 0) {
                    std::string dest;
                    dest.resize(buffer_size(b1));
                    auto [ec, n] = co_await rs.read_some(make_buffer(dest));
                    BOOST_TEST(! ec);
                    BOOST_TEST_EQ(n, buffer_size(b1));
                }
            }
        });
        BOOST_TEST(r.success);
