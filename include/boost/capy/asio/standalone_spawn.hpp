...
#ifndef BOOST_CAPY_STANDALONE_ASIO_SPAWN_HPP
#define BOOST_CAPY_STANDALONE_ASIO_SPAWN_HPP
...
struct standalone_asio_init;



template<typename Allocator>
struct standalone_asio_promise_type_allocator_base
{
  template<typename Handler, Executor Ex, IoRunnable Runnable>
  void * operator new   (std::size_t n, standalone_asio_init &, 
                         Handler & handler, 
                         Ex & exec, Runnable & r)
  {
...
template<>
struct standalone_asio_promise_type_allocator_base<std::allocator<void>>
{
};

template<typename Handler, Executor Ex, IoRunnable Runnable>
struct standalone_asio_init_promise_type 
  : standalone_asio_promise_type_allocator_base<::asio::associated_allocator_t<Handler>>
{
    using args_type = completion_tuple_for_io_runnable<Runnable>;
  
    standalone_asio_init_promise_type(standalone_asio_init &, Handler & h, Ex & exec, Runnable & r)  
        : handler(h), ex(exec) {}
    ...
    {
      Handler &handler;
      standalone_asio_executor_adapter<Ex> ex;
      args_type args;
      
      bool await_ready() const {return false;}
      void await_suspend(std::coroutine_handle<standalone_asio_init_promise_type> h)
      {
        ...
        auto handler =         
            std::apply( 
              [&](auto ... args) {return ::asio::append(std::move(h_), std::move(args)...);},
              args_);
        
        auto exec = ::asio::get_associated_immediate_executor(handler, ex);
    ...
    struct wrapper
    {
      ...
      std::coroutine_handle<> await_suspend(std::coroutine_handle<standalone_asio_init_promise_type> tr)
      {
        // always post in
        auto h = r.handle();
        auto & p = h.promise();
        ...
        cancel_slot = ::asio::get_associated_cancellation_slot(tr.promise().handler);
...
template<Executor ExecutorType, IoRunnable Runnable,
         ::asio::completion_token_for<detail::completion_signature_for_io_runnable<Runnable>> Token
            = ::asio::default_completion_token_t<ExecutorType>>
auto standalone_asio_spawn(ExecutorType exec, Runnable && runnable, Token token)
{
  return ::asio::async_initiate<Token, detail::completion_signature_for_io_runnable<Runnable>>(
            detail::standalone_asio_init{},
...
template<ExecutionContext Context, IoRunnable Runnable,
         ::asio::completion_token_for<detail::completion_signature_for_io_runnable<Runnable>> Token 
          = ::asio::default_completion_token_t<typename Context::executor_type>>
auto standalone_asio_spawn(Context & ctx, Runnable && runnable, Token token)
{
...
struct std::coroutine_traits<void, boost::capy::detail::standalone_asio_init&, Handler, Executor, Runnable>
{
  using promise_type = boost::capy::detail::standalone_asio_init_promise_type<Handler, Executor, Runnable>;
};
