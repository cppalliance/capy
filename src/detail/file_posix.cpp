...
#include <boost/capy/detail/file_posix.hpp>
...
#if ! defined(BOOST_CAPY_NO_POSIX_FADVISE)
# if defined(__APPLE__) || (defined(__ANDROID__) && (__ANDROID_API__ < 21))
#  define BOOST_CAPY_NO_POSIX_FADVISE
# endif
#endif

#if ! defined(BOOST_CAPY_USE_POSIX_FADVISE)
# if ! defined(BOOST_CAPY_NO_POSIX_FADVISE)
#  define BOOST_CAPY_USE_POSIX_FADVISE 1
# else
#  define BOOST_CAPY_USE_POSIX_FADVISE 0
# endif
#endif
...
open(char const* path, file_mode mode, system::error_code& ec)
{
    ...
    int f = 0;
#if BOOST_CAPY_USE_POSIX_FADVISE
    int advise = 0;
#endif
    switch(mode)
    {
