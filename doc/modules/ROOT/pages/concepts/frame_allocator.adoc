...
structures holding local variables, parameters, and suspension state. A frame
allocator must be a cheaply copyable handle to an underlying memory resource
(e.g., a pointer to a pool).

The library copies the allocator into the first coroutine frame for lifetime
safety. Subsequent frames in the call tree use the embedded allocator for
both allocation and deallocation.

== Default Frame Allocator

The library provides `default_frame_allocator` which passes through to
...
* xref:../execution/frame-allocation.adoc[Frame Allocation] — Tutorial on custom allocators
* xref:../coroutines/launching.adoc[Launching Tasks] — Using allocators with `async_run`
