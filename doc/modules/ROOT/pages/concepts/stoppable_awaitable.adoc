...
has a stop token, it passes the token to any stoppable awaitables it
awaits.
...
The awaitable should use the stop token to support cancellation of
...
| `a.await_suspend(h, d)`
| (unspecified)
| Suspends without cancellation support

| `a.await_suspend(h, d, token)`
| (unspecified)
| Suspends with cancellation support via the stop token
