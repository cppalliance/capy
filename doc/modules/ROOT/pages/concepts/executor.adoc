...
determining how submitted coroutines are to be executed. An executor is a
lightweight, copyable handle to an execution context such as a thread pool,
I/O context, or strand.
...
functions shall not introduce data races as a result of concurrent calls
...
* xref:is_execution_context.adoc[is_execution_context] — Execution context concept
* xref:dispatcher.adoc[dispatcher] — Simpler scheduling interface
* xref:../execution/executors.adoc[Executors] — Tutorial on the execution model
