...
* `data()` — Buffer sequence view of the remaining bytes (pass to `read_some`/`write_some`)
* `consume(n)` — Advance past `n` transferred bytes, in place
...
2. The slice views produced by `buffer_slice` and `consuming_buffers::data()` need to adjust the first and last buffers' bounds
