...
This page explains how to cancel running coroutines using `capy::stop_token`.

NOTE: Code snippets assume `using namespace boost::capy;` is in effect.

== Cooperative Cancellation

Capy supports cooperative cancellation through `capy::stop_token`. When a task
...
struct stoppable_timer
{
        ...
        capy::stop_token token)
    {
        ...
        capy::stop_callback cb(token, [timer_handle] {
...
any_coro await_suspend(any_coro cont, D const& d, capy::stop_token token)
{
