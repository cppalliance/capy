...
The program links no GUI library. It uses a stand-in instead.
...
`count_rows` runs on the pool. The line after the first `co_await` touches a widget, so it must run on the GUI thread. It does, and the source contains no hop. The second `co_await` is the toolkit's dialog, covered next.
