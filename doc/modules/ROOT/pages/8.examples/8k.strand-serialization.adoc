...
A cpp:strand[] wraps an executor and guarantees that handlers dispatched through it never run concurrently.
...
The strand serializes execution so only one coroutine runs at a time, preventing data races.
