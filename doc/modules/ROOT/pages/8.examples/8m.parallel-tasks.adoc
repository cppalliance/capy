...
cpp:when_all[] starts all four tasks concurrently on the thread pool. The result is cpp:io_result[io_result]`<long long, long long, long long, long long>`, a single `ec` plus the four partial sums in input order.
...
On a multi-core system you see different thread IDs, confirming true parallel execution. The `ostringstream` ensures each line is printed atomically.
