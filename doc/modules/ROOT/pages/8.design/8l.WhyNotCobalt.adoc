...
Cobalt defines stream abstractions (`write_stream`, `read_stream`, `stream`) as abstract base classes in `cobalt/io/stream.hpp`, taking a different approach from Capy's concept + type-erased wrapper model. See xref:8.design/8m.WhyNotCobaltConcepts.adoc[Write Stream Design] for a side-by-side analysis.
...
When algorithms operate on type-erased interfaces, testing becomes deterministic. Capy provides mock implementations for every stream concept. Cobalt defines stream abstractions as abstract base classes but does not provide mock implementations for testing. See xref:8.design/8m.WhyNotCobaltConcepts.adoc[Write Stream Design] for a comparison of the two stream designs.
