...
A `flat_dynamic_buffer` uses a single contiguous memory region:
...
flat_dynamic_buffer buf1;

// From storage with capacity
flat_dynamic_buffer buf2(storage, capacity);

// From storage with initial readable size
flat_dynamic_buffer buf3(storage, capacity, initial_size);
----

== circular_dynamic_buffer

A `circular_dynamic_buffer` uses a ring buffer that wraps around:
...
// Same interface as flat_dynamic_buffer
...
A `string_dynamic_buffer` adapts a `std::string` as a dynamic buffer:

[source,cpp]
----
std::string str;
string_dynamic_buffer buf(&str);
...
// Basic usage
string_dynamic_buffer buf1(&str);

// With maximum size limit
string_dynamic_buffer buf2(&str, 1024);  // Will not grow past 1024 bytes
----

=== Lifetime

The `string_dynamic_buffer` must not outlive the string it wraps:
