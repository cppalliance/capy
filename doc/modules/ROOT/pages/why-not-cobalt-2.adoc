...
`IoAwaitableTask` refines `IoAwaitable` with promise-level context storage. The promise must provide `set_environment`, `environment`, `set_continuation`, and their accessors:
...
concept IoAwaitableTask =
        ...
        coro cont)
    {
        ...
        { p.set_continuation(cont, env.executor) } noexcept;
