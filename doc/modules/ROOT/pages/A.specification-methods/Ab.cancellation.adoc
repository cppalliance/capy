...
can be prematuerly stopped using the `std::stop_token` propagated through the 
`IoAwaitable` protocol. Additionally, If the result type of expression `co_await a`
in the context of a Capy-coroutine is a specialization of `io_result` then the cancelling of operation is 
