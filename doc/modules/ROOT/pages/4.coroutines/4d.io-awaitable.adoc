...
----
template<typename T>
concept IoAwaitable = requires(T& t, coro h, io_env const& env) {
    { t.await_ready() } -> std::convertible_to<bool>;
    { t.await_suspend(h, env) } -> std::same_as<coro>;
    t.await_resume();
};
----
...
* `set_environment(io_env const&)` — Store the propagated execution environment
