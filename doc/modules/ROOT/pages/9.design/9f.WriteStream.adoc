...
cpp:WriteStream[] is the fundamental partial-write primitive among Capy's stream concepts.
...
Consider relaying 100KB from a cpp:ReadStream[] to a TCP socket. The kernel's send buffer accepts at most 40KB per call.
...
Each chunk is drained to completion before the caller can read more from the source.
