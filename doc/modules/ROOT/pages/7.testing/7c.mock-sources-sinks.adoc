...
any mismatch between written bytes and that prefix causes the next write
(`write`, `write_some`, or `write_eof(buffers)`) to return `error::test_failure`. Because `fuse` copies share state (see
xref:7.testing/7a.drivers.adoc#_shared_state_across_copies[Shared State Across Copies]),
