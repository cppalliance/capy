...
`data()`. The fuse is a separate concern used only for error injection.
Because `fuse` copies
share state (see
xref:7.testing/7a.drivers.adoc#_shared_state_across_copies[Shared State Across Copies]),
...
cpp:test::make_stream_pair[]`(f)` to test client/server code without real
sockets.
