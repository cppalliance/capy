...
check the token up front. A connected cpp:test::stream[stream] whose `read_some` is *blocked*
...
handle a failure at every async step?"). A typical test suite pairs both:
...
worker and you need to identify which worker was involved.
