...
untested until production breaks them. Capy replaces the transport with
in-memory mocks. Because each mock satisfies the same concept as its
...
then exception mode. Buffer utilities such as `bufgrind` and `buffer_to_string`
