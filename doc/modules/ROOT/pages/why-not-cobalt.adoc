...
Capy uses `std::stop_token`, propagated as a parameter via the IoAwaitable protocol. The token is part of the `await_suspend` signature — it flows forward structurally alongside the executor.
...
| Propagation
| Automatic (IoAwaitable parameters)
