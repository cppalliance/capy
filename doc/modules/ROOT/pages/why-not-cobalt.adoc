...
Cobalt's `cobalt::io` namespace provides wrappers around Asio I/O objects. These wrappers expose concrete Asio types through their interfaces. A `cobalt::io::steady_timer` is an `asio::basic_waitable_timer`. A `cobalt::io::socket` is an `asio::basic_stream_socket`. The wrappers do not hide the implementation; the underlying Asio types remain visible.
...
Capy defines seven coroutine-only stream concepts. Cobalt inherits Asio's `AsyncReadStream` and `AsyncWriteStream`, which are hybrid concepts supporting callbacks, futures, and coroutines. Cobalt's `cobalt::io` wrappers simplify the API but define no new stream abstractions and still include full Asio headers.
...
Cobalt does not define stream abstractions, so type erasure at this level is not applicable.
...
Capy introduces the https://github.com/cppalliance/wg21-papers/blob/master/source/d4003-io-awaitables.md[IoAwaitable protocol] and uses it for context propagation. When you `co_await`, the caller passes its executor and stop token to the child structurally:
...
Capy propagates `std::stop_token` automatically through coroutine chains via the IoAwaitable protocol. Cancel at the top of your coroutine tree, and every nested operation receives the signal. Pending I/O operations cancel at the OS level — `CancelIoEx` on Windows, `IORING_OP_ASYNC_CANCEL` on Linux.

Cobalt uses Asio's `cancellation_signal` and `cancellation_slot`. Cancellation of pending I/O operations is handled by Asio at the OS level. Propagation through the coroutine chain requires explicit management via `this_coro::cancellation_state`.
...
| Propagation
| Automatic (IoAwaitable protocol)
| Manual (`this_coro::cancellation_state`)
...
| Cancellation
| Automatic propagation, OS-level
| Manual propagation, OS-level (via Asio)
