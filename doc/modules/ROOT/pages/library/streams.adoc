...
**Inherits from Base**::
`owning<Base, T>` inherits from `Base`, so it can be used anywhere `Base&`
is accepted.

**Owns Instance of T**::
The wrapper constructs and owns an instance of `T`. Constructor arguments
are forwarded to `T`'s constructor.

**Move Semantics**::
Move operations transfer both the owned object and the type eraser state.
The base is automatically rebound to the new object location.
