...
concept IoAwaitableTask = IoAwaitable<T> && requires
{
    ...
    requires requires(typename T::promise_type& p, io_env const& env)
    {
        p.set_environment(env);
        { p.environment() } -> std::same_as<io_env const&>;
    };
};
----
